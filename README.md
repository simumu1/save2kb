...
### 场景 A：国内反爬网站（公众号/知乎等）→ 走浏览器 compact snapshot

这类网站用 curl/web_extract 抓不到，必须走浏览器。浏览器整页 HTML 巨大，但 compact snapshot 只取文字。

| 读取方式 | 数据量 | 约合 Token | 倍数 |
|:---------|:------|:-----------|:----:|
| 🔴 浏览器整页 HTML（含 JS/CSS/跟踪代码） | **4,055,779 字符** | 数百万+ | **1,429x** |
| 🟢 **save2kb（compact snapshot → 纯文字 MD）** | **2,838 字符** | **~2,000-4,000** | **1x** |

> 📍 实测：一篇公众号文章（《每日SEO 198》），`outerHTML` vs `innerText`
> **节省 99.93% token**。这类网站无论文章长短，HTML 固定开销占 95%+。

### 场景 B：国际/开放网站（The Verge/Simplified/Ars等）→ 走 web_extract

这类网站 curl 可直接抓。save2kb 优先用 web_extract 读取，只取纯文字。

| 网站 | 原始 HTML | 纯文字 | 节省 |
|:----|:---------:|:------:|:----:|
| The Verge（科技长篇） | 632,418 | 250,901 | **60%** |
| Simplified（博客） | 280,269 | 138,012 | **51%** |
| Ars Technica（科技新闻） | 1,991 | 1,213 | **39%** |

> **平均节省约 50%**。国际网站 HTML 相对干净，但去掉标签后的纯文字仍然省一半 token。

### 结论总结

| 场景 | 节省幅度 | 原因 |
|:----|:--------:|:-----|
| 🏠 国内反爬站（公众号/知乎等） | **~99.9%** | 浏览器整页含巨量 JS → compact snapshot 只取文字 |
| 🌍 国际开放站 | **~40-60%** | curl 抓取 → 去掉 HTML 标签的纯文字 |
| 📄 你自己网站上的干净页面 | **~20-40%** | 本身 HTML 就精简，但去掉导航/侧栏仍有节省 |
